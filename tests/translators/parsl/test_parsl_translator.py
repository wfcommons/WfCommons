...
class TestParslTranslator:

    def make_tarfile_of_wfcommons(self):
    ...
    def test_parsl_translator(self) -> None:
        ...
        # Make sure the target dir does not already exist
        ...
        image_name = "wfcommons/wfcommons-testing-dask"
        ...
        container = client.containers.run(
            image_name,
            "sleep infinity",
            volumes={str_dirpath: {'bind': str_dirpath, 'mode': 'rw'}},
            working_dir=str_dirpath,
            tty=True,
            detach=True
        )
        ...
        tar_data = self.make_tarfile_of_wfcommons()
        ...
        exit_code, output = container.exec_run("sudo python3 -m pip install -e . --break-system-packages", workdir="/tmp/WfCommons", stdout=True, stderr=True)

        # Copy over the wfbench and cpu-benchmark executables
