...
class HierarchicalPegasusLogsParser(LogsParser):
    ...
    def __init__(self,
        ...
        raise ValueError("Hierarchical Pegasus log parsing has been deprecated (code is commented out for now)")
    ...
    #                 size=0,
    #                 logger=self.logger
    #             ) for f in j.findall('{http://pegasus.isi.edu/schema/DAX}uses') if f.get('link') == "input"]
    #
    #             output_files = [File(
    #                 file_id=f.get('name') if not f.get('name') is None else f.get('file'),
    #                 size=0,
    #                 logger=self.logger
    #             ) for f in j.findall('{http://pegasus.isi.edu/schema/DAX}uses') if f.get('link') == "output"]
    #
    #             self.workflow.add_node(
    #                 task_name,
    #                 task=Task(
