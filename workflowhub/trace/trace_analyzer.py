...
class TraceAnalyzer:
    ...
    def generate_fit_plots(self, trace_element: TraceElement, outfile_prefix: Optional[str] = None) -> None:
        ...
        """
        self.logger.info('Generating fit plots ({}).'.format(trace_element.value))
        outfile_prefix = outfile_prefix + '_' if outfile_prefix else ''

        for job_summary in self.traces_summary:
            outfile = outfile_prefix + job_summary.lower() + '-' + trace_element.value
            el = self.traces_summary[job_summary][trace_element.value]

            if trace_element == TraceElement.RUNTIME:
                _generate_fit_plots(el, job_summary + ' (' + trace_element.value + ')',
                                    outfile=outfile + '.png', logger=self.logger)
            else:
                for k in el:
                    ext = k if '.' not in k else k[1:]
                    _generate_fit_plots(el[k], job_summary + ' (' + trace_element.value + '): ' + ext,
                                        outfile=outfile + '-' + ext + '.png', logger=self.logger)
...
def _generate_fit_plots(el: Dict, title: str, outfile: str, logger: Optional[Logger] = None) -> None:
